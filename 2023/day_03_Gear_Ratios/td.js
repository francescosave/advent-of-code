const paragraph = `........................................................862...........20.............453...619......58........694...312.................292.
...
rows.forEach((row, rowIndex) => {
  const numbersInString = row.match(/([0-9]+)/gm);

  console.log(numbersInString)

  let startPosition = 0;
  if (!numbersInString) return;
  
  numbersInString.forEach(number => {

    console.log("number", number);
    
    startPosition = row.indexOf(number, startPosition);
    const numberLength = number.length;



    const startIndex = startPosition - 1 === -1 ? 0 : startPosition - 1; 
    const endIndex = numberLength + startPosition + 1 > row.length ? row.length - 1 : numberLength + startPosition + 1;
    
    if ((row[startIndex] && row[startIndex].match(/[^\.0-9]/gm)) || (row[endIndex - 1] && row[endIndex - 1].match(/[^\.0-9]/gm))) {
      validNumbers = validNumbers + (number * 1);
      arrayOfValidNumbers.push(number)
      return
    }

    console.log("Symbols not in the same row" + row[startIndex] + " " + row[endIndex])
    
    
    const rowBefore = rows[rowIndex - 1];
    const rowAfter = rows[rowIndex + 1];

    const before = rowBefore ? rowBefore.slice(startIndex, endIndex) : ""; 
    
    if (before.match(/[^\.0-9]/gm)) {
      validNumbers = validNumbers + (number * 1);
      arrayOfValidNumbers.push(number)
      return;
    }

    console.log("Symbols not in the previous row")
    console.log(before) 

    const after = rowAfter ? rowAfter.slice(startIndex, endIndex) : "";

    console.log(rowAfter)

    if (after.match(/[^\.0-9]/gm)) {
      validNumbers = validNumbers + (number * 1);
      arrayOfValidNumbers.push(number)
      return;
    }

    console.log("Symbols not in the next row")
    console.log(after)    
  })
})

console.log(validNumbers)
console.log(arrayOfValidNumbers)
//const numbersInString = found.map(element => element * 1);
